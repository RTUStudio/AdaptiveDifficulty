...
public class PlayerDamageByEntity extends RSListener<AdaptiveDifficulty> {
    ...
    private void onPlayerDamageByEntity(EntityDamageByEntityEvent e) {
        if (e.getEntity() instanceof Player player) {
            Difficulty difficulty = config.get(manager.get(player.getUniqueId()));
            if (difficulty == null) return;
            
            if (!difficulty.damage().explosion() && explosions.stream().anyMatch(type -> type.equalsIgnoreCase(e.getDamager().getType().name()))) {
                e.setCancelled(true);
                return;
            }
